...
void CV32E40P_DSE_PerformanceModel::connectChannel(Channel* channel_)
{
  ...
  iCache.pc_ptr = channel->pc;

  divider.rs2_data_ptr = channel->rs2_data;

  divider_u.rs2_data_ptr = channel->rs2_data;

  dCache.addr_ptr = channel->addr;

}

uint64_t CV32E40P_DSE_PerformanceModel::getCycleCount(void)
{
...
std::string CV32E40P_DSE_PerformanceModel::getPipelineStream(void)
{
...
std::string CV32E40P_DSE_PerformanceModel::getPrintHeader(void)
{
  std::stringstream ret_strs;
  ret_strs << "Enter";
  ret_strs << "," << "IF_stage";
