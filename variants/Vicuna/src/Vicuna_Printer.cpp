...
extern InstructionPrinterSet* Vicuna_InstrPrinterSet;

Vicuna_Printer::Vicuna_Printer(): Printer("Vicuna_Printer", Vicuna_InstrPrinterSet)
{}

void Vicuna_Printer::connectChannel(Channel* ch_)
{
  Vicuna_Channel* channel = static_cast<Vicuna_Channel*>(ch_);
  ...
  isWidening_ptr = channel->isWidening;
  pc_ptr = channel->pc;
...
std::string Vicuna_Printer::getPrintHeader(void)
{
  ...
  caption_strs << std::setfill(' ') << std::setw(18) << std::left << "isWidening" << " | ";
