...
void Vicuna_zvl64b_PerformanceModel::connectChannel(Channel* channel_)
{
  ...
  vectorMemRegistersModel.nf_ptr = channel->nf;
  ...
  packUnpackLsu.vtype_ptr = channel->vtype;
  packUnpackLsu.lsWidth_ptr = channel->lsWidth;
...
std::string Vicuna_zvl64b_PerformanceModel::getPipelineStream(void)
{
  ...
  ret_strs << "," << EX_stage;
  ret_strs << "," << WB_stage;
  ret_strs << "," << V_IQ_stage.get(1);
  ret_strs << "," << V_DISP_stage.get(1);
  ret_strs << "," << V_EX_stage.get(1);
  ret_strs << "," << V_WB_stage;
  ret_strs << "," << V_RES_stage;
  ret_strs << "," << OFF_IQ_stage.get(1);
  ret_strs << "," << OFF_SIG_stage;
  ret_strs << std::endl;
  return ret_strs.str();
}
