...
bool BranchHistoryTabel::getPrediction(uint64_t pc_, uint64_t imm_) const {
    if(!tabel[indexTabel(pc_)].valid){
        // Predict taken if imm is negative
        return (int64_t(imm_) < 0);
    }
    return tabel[indexTabel(pc_)].state.getPrediction();
}

void BranchHistoryTabel::update(uint64_t pc_, bool taken_){
    tabel[indexTabel(pc_)].valid = true;
    tabel[indexTabel(pc_)].state.update(taken_);
}
