...
{
  public:
    VectorRegisterModel(PerformanceModel *parent_) : ConnectorModel("VectorRegisterModel", parent_){};
    ...
    uint64_t getVs1(void) { return vectorRegisterModel[vs1_ptr[getInstrIndex()]]; };
    uint64_t getVs2(void) { return vectorRegisterModel[vs2_ptr[getInstrIndex()]]; };
    uint64_t getVs3(void) { return vectorRegisterModel[vs3_ptr[getInstrIndex()]]; };
    ...
    void setVdGroupLoad(uint64_t baseTimestamp_)
    {
        ...
        else if (loadWidth < sew)
        {
            ...
            auto decrement = log2(loadWidth / sew);
        ...
        {
            vectorRegisterModel[registerBaseIndex + i] = baseTimestamp_ + ((i + 1) * cyclesPerRegister);
        }
    }
    ...
    {
        static constexpr auto cyclesPerRegister = VectorConfig::vlen / VectorConfig::vLaneWidth;
    ...
    void setVdGroupDivider(uint64_t baseTimestamp_)
    {
        ...
        auto cyclesPerRegister = (VectorConfig::vlen / getSew()) * VectorConfig::dividerDelay;
    ...
    void setVd(uint64_t vd_) { vectorRegisterModel[vd_ptr[getInstrIndex()]] = vd_; };
